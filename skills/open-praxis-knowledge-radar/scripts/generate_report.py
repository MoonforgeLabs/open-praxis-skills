#!/usr/bin/env python3
"""生成知识雷达报告 - Markdown 或 HTML 格式

用法:
    python3 generate_report.py                    # 生成 Markdown 报告
    python3 generate_report.py --format html      # 生成 HTML 报告
    python3 generate_report.py --output report.md # 指定输出文件
    python3 generate_report.py --open             # 生成后打开
"""
...
def generate_markdown_report(tasks: list[dict[str, Any]], detailed: bool = True) -> str:
    ...
    for status in status_order:
        ...
        if detailed:
            # 详细模式：显示完整信息
            lines.append(f"| # | 优先级 | 领域 | 标题 | 来源 | 时间 |")
            lines.append(f"|---|--------|------|------|------|------|")
            ...
            for i, task in enumerate(status_tasks[:20], 1):  # 最多显示 20 条
                ...
                lines.append(f"| {i} | {priority_emoji} {priority} | {area_emoji} {area} | {title} | {source} | {created} |")
    ...
    for task in recent_tasks:
        ...
        if url:
            lines.append(f"- {area_emoji} **[{title}]({url})** ({source}, {created})")
        else:
            lines.append(f"- {area_emoji} **{title}** ({source}, {created})")
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="progress-bar">
                    <div class="progress-fill" style="width: {(status_counts.get('inbox', 0) + status_counts.get('next', 0)) / len(tasks) * 100 if tasks else 0}%"></div>
                </div>
            </div>
            <div class="stat-card">
                <h3>📖 学习中</h3>
...
"""
    ...
    for task in recent_tasks:
        ...
        if url:
            html += f"""
                <li class="url-item">
                    <span class="url-icon">{area_emoji}</span>
                    <span class="url-title"><a href="{url}" target="_blank">{title}</a></span>
                    <span class="url-meta">{source} · {created}</span>
                </li>
"""
        else:
            html += f"""
...
"""
    ...
    for status in status_order:
        ...
        for task in status_tasks[:10]:
            ...
            html += f"""
                <li class="task-item">
                    <span class="task-badge badge-{status}">{emoji} {status}</span>
                    <span class="task-priority">{priority_emoji}</span>
                    <span class="task-area">{area_emoji} {area}</span>
...
"""
        ...
        if len(status_tasks) > 10:
            html += f"""
                <li class="task-item">
                    <span class="task-title" style="color: #999; text-align: center; width: 100%;">
