...
def main():
    ...
    if config["output"]:
        ...
        output_dir = Path.home() / ".iot-common-skills" / "data" / "ai-usage-stats" / "reports"
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>📊 Generated by ai-usage-stats | 双击打开，无需服务器</p>
        </div>
    </div>
    ...
    <script>
        ...
        // 图表实例
        let trendChart, kindChart, runtimeChart, skillsChart, projectsChart;
        ...
        function calcStats(events) {{
            ...
            const byProject = {{}};
            events.forEach(e => {{
                const p = e.project || '(未分类)';
                if (!byProject[p]) byProject[p] = {{ count: 0, tokens: 0, cost: 0 }};
                byProject[p].count++;
                byProject[p].tokens += (e.tokens_in || 0) + (e.tokens_out || 0);
                byProject[p].cost += e.cost_usd || 0;
            }});
            const topProjects = Object.entries(byProject)
                .sort((a, b) => b[1].cost - a[1].cost)
                .slice(0, 10);

            return {{ total, tokens, cost, byKind, byRuntime, byDate, topSkills, topProjects }};
        }}
        ...
        // 更新项目费用图
        function updateProjectsChart(topProjects) {{
            const labels = topProjects.map(p => p[0]);
            const data = topProjects.map(p => p[1].cost);
        ...
        function refresh() {{
            ...
            updateProjectsChart(stats.topProjects);
