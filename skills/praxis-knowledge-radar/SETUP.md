...
> **注意**: 钉钉（DingTalk）配置部分为区域特定功能（主要面向中国大陆用户）。
> 如果你使用其他通知系统（如 Slack、Discord、企业微信等），请跳过钉钉相关章节，
> 参考"通知插件配置"部分替换为你使用的系统。

## 1. 通知插件配置（可选）

### 配置状态（钉钉示例）

> 以下是钉钉 MCP 的具体配置步骤，作为示例供参考。
> 你可以替换为你实际使用的通知系统。
...
## 2. Webhook 配置（可选，简单方式）

> 以下以钉钉 Webhook 为例，你可以替换为其他系统的 Webhook（如 Slack Incoming Webhook 等）。
...
```bash
# 每周一早上 9 点（已配置）
0 9 * * 1 <your-skill-path>/scripts/scheduled_sync.sh

# 每天早上 9 点
0 9 * * * <your-skill-path>/scripts/scheduled_sync.sh

# 每周一和周四早上 9 点
0 9 * * 1,4 <your-skill-path>/scripts/scheduled_sync.sh
```
...
- **通知配置**: `~/.praxis-skills/data/knowledge-radar/dingtalk-config.json`（钉钉示例）
